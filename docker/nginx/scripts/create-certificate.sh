...
set -f
PATH_SSL="/etc/nginx/certs"

# Path to the custom Docker $(hostname) Root CA certificate.
PATH_ROOT_CNF="${PATH_SSL}/ca.docker.dev.cnf"
PATH_ROOT_CRT="${PATH_SSL}/ca.docker.dev.crt"
PATH_ROOT_KEY="${PATH_SSL}/ca.docker.dev.key"
...
then
    ...
    # Finally, generate the private key and certificate signed with the Homestead dev Root CA.
